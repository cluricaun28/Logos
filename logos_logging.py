"""Centralized logging setup for Logos.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.hermes/logs/`` (profile-aware via ``get_logos_home()``).
...
from logos_constants import get_config_path, get_logos_home
...
# Used by _ComponentFilter and exposed for ``logos logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from logos_cli.config import is_managed
