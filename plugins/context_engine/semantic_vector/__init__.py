...
class SemanticVectorContextEngine(ContextEngine):
    ...
    def __init__(self, **kwargs):
        ...
        if not self._model_path:
            self._model_path = os.path.expanduser("~/.hermes/models/embeddings/all-MiniLM-L6-v2")
    ...
    def archive(
        ...
        if engine is not None:
            try:
                return self._semantic_archive(messages, focus_topic, current_tokens)
            except Exception as e:
                logger.warning("Semantic archive failed, falling back to tail-off: %s", e)
    ...
    def _semantic_archive(
        ...
        for i in range(max(0, len(messages) - self.protect_last_n), len(messages)):
            keep_indices.add(i)
        ...
        effective_tokens = current_tokens if current_tokens is not None else estimated_tokens
        ...
        if not self.should_archive(effective_tokens):
            ...
            if state_map and result:
                # Inject state map into last assistant message or prepend
                self._inject_state_map(result, state_map)
            return result

        # Build result from kept indices
        result = [messages[i] for i in sorted(keep_indices) if i < len(messages)]
        ...
        if result_tokens > self.threshold_tokens and self.threshold_tokens > 0:
            logger.info(
                "Semantic pruning insufficient (%d > %d tokens), applying tail-off fallback",
                result_tokens, self.threshold_tokens,
            )
    ...
    def _tailoff_archive(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _assign_turn_to_vector(self, turn_idx: int, text: str, embedding: List[float] = None):
        ...
        if best_vec is not None and best_sim >= self.similarity_threshold:
            ...
            for j in range(len(best_vec.centroid)):
                best_vec.centroid[j] = (best_vec.centroid[j] * (n - 1) + embedding[j]) / n
            # Update name from text
            words = text[:80].split()[:5]
            best_vec.name = "-".join(words) if words else f"topic-{best_vec.vector_id}"
        else:
            # Create new vector
            words = text[:80].split()[:5]
            name = "-".join(words) if words else f"topic-{self._next_vector_id}"
    ...
    def _build_state_map(self) -> str:
        ...
        for vec in sorted(self._vectors, key=lambda v: v.last_seen_turn, reverse=True):
            lines.append(
    ...
    def _inject_state_map(self, messages: List[Dict[str, Any]], state_map: str):
    ...
    def _turn_to_msg_index(self, turn_idx: int, messages: List[Dict[str, Any]]) -> int | None:
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            if os.path.isdir(self._model_path):
                ...
                logger.info("SemanticVectorContextEngine: loaded local model from %s on CPU", self._model_path)
                return True
            else:
                logger.warning("SemanticVectorContextEngine: local model not found at %s", self._model_path)
                return False
        except ImportError:
            logger.warning("SemanticVectorContextEngine: sentence-transformers not installed")
            return False
        except Exception as e:
            logger.warning("SemanticVectorContextEngine: failed to load model: %s", e)
            return False
    ...
    def compress(
        self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None
    ) -> List[Dict[str, Any]]:
    ...
    def on_session_start(self, session_id: str, **kwargs) -> None:
        ...
        # Try to load model on session start
        try:
            self._load_model()
        except Exception as e:
            logger.debug("Failed to load embedding model on session start: %s", e)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status["active_vectors"] = sum(1 for v in self._vectors if v.status == "Active")
        status["model_loaded"] = self.model is not None
        return status
