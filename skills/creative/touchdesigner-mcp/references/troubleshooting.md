...
## 5. Logos Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.logos/config.yaml` (legacy `~/.hermes/config.yaml`)` when `HERMES_HOME` is unset)
...
Restart the Logos session for changes to take effect. The MCP connection is
