#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Logos.
...
# Ensure sibling modules (_logos_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _logos_home import display_logos_home, get_logos_home
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Logos profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'logos[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Logos home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_logos_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Logos")
