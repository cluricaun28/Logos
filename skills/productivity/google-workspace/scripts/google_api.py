#!/usr/bin/env python3
"""Google Workspace API CLI for Logos.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Logos-facing JSON contract and falls back to the Python client
...
from _logos_home import get_logos_home
...
def _gws_binary() -> str | None:
    override = os.getenv("LOGOS_GWS_BIN") or os.getenv("HERMES_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Logos")
