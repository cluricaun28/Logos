...
- A long-running process (logos gateway) misbehaves and you can't restart it
...
The logos test runner and pytest both support this:
...
For long-lived processes: Logos gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Logos you have two practical options:
...
## Debugging Logos-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `logos` normally. Control returns to your terminal at the pause point.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Logos subagents, debug one process at a time.
