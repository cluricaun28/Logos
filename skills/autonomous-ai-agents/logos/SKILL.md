...
├── cli.py                # Interactive CLI (LogosCLI)
├── logos_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── logos_cli/           # CLI subcommands, config, setup, commands
...
All handlers must return JSON strings. Use `get_logos_home()` for paths, never hardcode `~/.hermes`.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `logos_cli/commands.py`
...
- Use `get_logos_home()` from `logos_constants` for all paths (profile-safe)
