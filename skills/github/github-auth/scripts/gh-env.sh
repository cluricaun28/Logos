#!/usr/bin/env bash
# GitHub environment detection helper for Logos skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "${LOGOS_HOME:-$HOME/.logos}/.env" ] && grep -q "^GITHUB_TOKEN=***" "${LOGOS_HOME:-$HOME/.logos}/.env" 2>/dev/null; then
    GITHUB_TOKEN=*** "^GITHUB_TOKEN=***" "${LOGOS_HOME:-$HOME/.logos}/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
elif [ -f "${HERMES_HOME:-$HOME/.hermes}/.env" ] && grep -q "^GITHUB_TOKEN=***" "${HERMES_HOME:-$HOME/.hermes}/.env" 2>/dev/null; then
    GITHUB_TOKEN=*** "^GITHUB_TOKEN=***" "${HERMES_HOME:-$HOME/.hermes}/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
