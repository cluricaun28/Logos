...
def _credentials_path() -> Path:
    return get_logos_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "Logos looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `logos login --provider google-gemini-cli` first.",
...
<html><head><meta charset="utf-8"><title>Logos — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Logos — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — Logos will walk you through a manual paste fallback.</p></body></html>
"""
