...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`logos_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``logos_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Logos-canonical
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``logos_cli.config.load_config``
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from logos_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Logos wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Logos
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`logos_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_logos_home() / ALLOWLIST_FILENAME
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Logos is about to register a shell hook that will run a\n"
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`logos_cli.plugins.invoke_hook`
    ...
    holding the canonical Logos-wire-shape response."""
