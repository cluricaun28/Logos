"""Unified removal contract for every credential source Logos reads from.

Logos seeds its credential pool from many places:
    ...
    manual        — user ran `logos auth add`
...
unify here is **removal**:
    ...
    ``logos auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    """
    from logos_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Logos.",
            f"  The pool entry is now suppressed — Logos will ignore "
            f"{env_var} until you run `logos auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Logos stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `logos auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    ...
    result = RemovalResult()
    oauth_file = get_logos_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Logos Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `logos login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `logos auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Logos auth store is not enough — _seed_from_singletons() would
    ...
    via ``logos auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from logos_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `logos auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `logos auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Logos stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `logos auth add copilot` to re-enable if needed.",
    ])
