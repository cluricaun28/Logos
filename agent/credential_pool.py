...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``logos model`` / ``logos auth`` which
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Logos profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `logos model` / `logos auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `logos auth remove <provider> <N>` is stable across all source types.
    try:
        from logos_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Logos PKCE)
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `logos auth add nous --label <name>`).  Fall back to the
        ...
        # Logos auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from logos_cli.auth import resolve_qwen_runtime_credentials
        ...
        # Respect user suppression — `logos auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Logos auth store.  Without this gate the removal is instantly
        ...
        # Logos owns its own Codex auth state — we do NOT auto-import from
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    changed = False
    active_sources: Set[str] = set()
    # Honour user suppression — `logos auth remove <provider> <N>` for an
    ...
    try:
        from logos_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
