"""
Tests for timezone support (logos_time module + integration points).
...
def _reset_logos_time_cache():
    """Reset the logos_time module cache (replacement for removed reset_cache)."""
    logos_time._cached_tz = None
    logos_time._cached_tz_name = None
    logos_time._cache_resolved = False


# =========================================================================
# logos_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="logos_time"):
            result = logos_time.now()
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != Logos tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to Logos tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Logos tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Logos tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Logos is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Logos tz
        ...
        # Use a Logos timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _logos_now's wall time — this would
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Logos timezone"
        )
