...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("logos_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("logos_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("logos_cli.main.select_provider_and_model", fake_select)
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import logos_cli.gateway as gateway_mod
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(logos_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("logos_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("logos_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "logos_cli.main._model_flow_named_custom",
...
def test_resolve_hermes_chat_argv_falls_back_to_module(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod.importlib.util, "find_spec", lambda name: object() if name == "logos_cli" else None)

    assert setup_mod._resolve_hermes_chat_argv() == [sys.executable, "-m", "logos_cli.main", "chat"]
