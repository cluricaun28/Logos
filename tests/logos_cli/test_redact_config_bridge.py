...
`HERMES_REDACT_SECRETS` at module-import time. `logos_cli/main.py` at
...
env var in `logos_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports logos_cli.main and checks
