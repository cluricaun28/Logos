...
from logos_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """logos_cli/main.py uses time.sleep(3) after systemctl restart to
        ...
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="hermes-gateway.service loaded active running Logos Gateway\n",
                    stderr="",
                )
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "logos_cli.gateway._graceful_restart_via_sigusr1",
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="hermes-gateway.service loaded inactive dead Logos Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.hermes/hermes-agent/venv/bin/python -m logos_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.hermes/hermes-agent/venv/bin/python -m logos_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
...
class TestCmdUpdateLegacyGatewayWarning:
    ...
    Users who installed Logos before the service rename often have a
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Logos gateway" not in captured
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Logos gateway" in captured
