...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ...
        ("logos_cli.setup.save_config", {}),
        ("logos_cli.setup.get_env_value", {"return_value": None}),
        ("logos_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("logos_cli.setup._print_setup_summary", {}),
        ("logos_cli.setup._offer_launch_chat", {}),
        ("logos_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("logos_cli.setup.load_config", {"return_value": {}}),
        ("logos_cli.setup.save_config", {}),
        ("logos_cli.auth.get_active_provider", {"return_value": None}),
        ("logos_cli.setup.get_env_value", {"return_value": None}),
        ("logos_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="logos_cli.setup.prompt_choice",
                quick="logos_cli.setup._run_quick_setup",
                model="logos_cli.setup.setup_model_provider",
                terminal="logos_cli.setup.setup_terminal_backend",
                agent="logos_cli.setup.setup_agent_settings",
                gateway="logos_cli.setup.setup_gateway",
                tools="logos_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("logos_cli.setup.prompt_choice", {"return_value": 0}),
                first="logos_cli.setup._run_first_time_quick_setup",
            )
            from logos_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "logos_cli.setup.run_setup_wizard",
