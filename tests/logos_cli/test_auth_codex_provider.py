"""Tests for Codex auth — tokens stored in Logos auth store (~/.hermes/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Logos auth store."""
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Logos auth store, not ~/.codex/."""
    hermes_home = tmp_path / "hermes"
    codex_home = tmp_path / "codex-cli"
    hermes_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Logos store
    assert not (codex_home / "auth.json").exists()

    # Logos auth store should have the tokens
    data = _read_codex_tokens()
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("logos_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "logos_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("logos_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("logos_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
