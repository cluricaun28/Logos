...
``_finalize_update_output`` in ``logos_cli/main.py``.  These exist so
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_logos_home if present
        import logos_cli.config as _cfg
        if hasattr(_cfg, "_LOGOS_HOME_CACHE"):
            _cfg._LOGOS_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "logos update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_logos_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "logos_cli.config.get_logos_home", _boom, raising=True
        )
