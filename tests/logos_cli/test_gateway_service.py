...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.delenv("LOGOS_RESTART_DRAIN_TIMEOUT", raising=False)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: logos gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup logos gateway" in out
...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HERMES_HOME=/home/alice/.logos' in unit
    ...
    def test_user_unit_unaffected_by_change(self):
        ...
        hermes_home = str(gateway_cli.get_logos_home().resolve())
        assert f'HERMES_HOME={hermes_home}' in unit


class TestHermesHomeForTargetUser:
    """Unit tests for _logos_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("HERMES_HOME", raising=False)

        result = gateway_cli._logos_home_for_target_user("/home/alice")
        assert result == "/home/alice/.logos"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "logos gateway run" in msg  # foreground fallback mentioned
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gateway_cli, "get_logos_home", lambda: root_home / ".hermes")
...
class TestDockerAwareGateway:
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "logos gateway run" in out
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Logos Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m logos_cli.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m logos_cli.main gateway run
          - python path/to/logos_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m logos_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/logos_cli/main.py gateway run",
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "logos gateway migrate-legacy" in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo logos gateway migrate-legacy" in out
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import logos_cli.main as cli_main
        ...
        if project_root is None:
            import logos_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "logos_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "logos_cli.main", "gateway", "status", "-l", "--help"],
