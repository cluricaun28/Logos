...
def _make_hermes_tree(root: Path) -> None:
    ...
    (root / "logos_state.db").write_bytes(b"fake-state")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        from logos_cli.backup import _should_exclude
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Logos backup."""
        from logos_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only logos_state.db (old wrong name) is rejected."""
        from logos_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["logos_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from logos_cli.backup import run_backup, run_import
...
class TestFormatSize:
    def test_bytes(self):
        from logos_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        ...
        import io
        from logos_cli.backup import _detect_prefix
...
class TestProfileRestoration:
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import logos_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "logos_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from logos_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def test_list_snapshots(self, hermes_home):
        from logos_cli.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, hermes_home):
        from logos_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from logos_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from logos_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from logos_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for logos_cli.config + logos_constants so they pick up
        # HERMES_HOME. Reload in place (not delete+reimport) so other test
        # modules' `from logos_cli.config import ...` bindings stay live —
        # reload re-runs the module body in the same namespace/dict.
        _mods = __import__("sys").modules
        for mod in list(_mods.keys()):
            if mod.startswith("logos_cli.config") or mod == "logos_constants":
    ...
    def test_backup_flag_creates_backup(self, hermes_home, capsys):
        ...
        from logos_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, hermes_home, capsys):
        ...
        # other test modules' `from logos_cli.config import ...` bindings
        # stay live (their __globals__ is this same dict).
        for mod in list(_sys.modules.keys()):
            if mod.startswith("logos_cli.config"):
