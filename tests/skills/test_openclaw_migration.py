...
"""
    ...
    assert "Tyler Williams > Active Projects: Logos" in entries
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Logos``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Logos prefers Python 3.11"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Logos said to ask Logos about Logos settings"
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewritten to ``~/.Logos/config.yaml`` — a path that doesn't exist —
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Logos" in result
