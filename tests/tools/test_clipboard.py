...
class TestSaveClipboardImage:
    ...
    def test_dispatches_to_windows_on_win32(self, tmp_path):
        ...
        with patch("logos_cli.clipboard.sys") as mock_sys:
            ...
            with patch("logos_cli.clipboard._windows_save", return_value=False) as m:
                save_clipboard_image(dest)
                m.assert_called_once_with(dest)
...
class TestIsWsl:
    def setup_method(self):
        # _is_wsl is now logos_constants.is_wsl — reset its cache
        import logos_constants
        logos_constants._wsl_detected = None
...
class TestWindowsHasImage:
    ...
    def test_clipboard_has_image(self):
        with patch("logos_cli.clipboard._get_ps_exe", return_value="powershell"):
            with patch("logos_cli.clipboard.subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(stdout="True\n", returncode=0)
    ...
    def test_no_powershell_available(self):
        with patch("logos_cli.clipboard._get_ps_exe", return_value=None):
            assert _windows_has_image() is False
...
class TestHasClipboardImageWin32:
    ...
    def test_dispatches_on_win32(self):
        with patch("logos_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch("logos_cli.clipboard._windows_has_image", return_value=True) as m:
...
class TestHasClipboardImage:
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        with patch("logos_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("logos_cli.clipboard._is_wsl", return_value=True):
                with patch("logos_cli.clipboard._wsl_has_image", return_value=False) as wsl:
                    with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                        with patch("logos_cli.clipboard._wayland_has_image", return_value=True) as wl:
                            assert has_clipboard_image() is True
...
class TestPreprocessImagesWithVision:
    ...
    @pytest.fixture
    def cli(self):
        """Minimal LogosCLI with mocked internals."""
        with patch("cli.load_cli_config") as mock_cfg:
            ...
            with patch.dict("os.environ", {"OPENROUTER_API_KEY": "test-key"}):
                with patch("cli.CLI_CONFIG", mock_cfg.return_value):
                    ...
                    cli_obj = LogosCLI.__new__(LogosCLI)
