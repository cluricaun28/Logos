...
# Async delegation capacity — separate from sync batch concurrency.
# Controls how many background subagents can run simultaneously.
...
def delegate_task(
    ...
    if n_tasks == 1:
        ...
        # fresh turn via process_registry.completion_queue. Batch async is
        # intentionally NOT supported in v1 — the rejection is handled above.
        background_enabled = is_truthy_value(background, default=False)
        if background_enabled:
            ...
            if dispatch.get("status") == "dispatched":
                # Detach background child from parent's active children list.
                # Without this, parent-turn interrupts would kill the detached
                # subagent mid-run, defeating the point of background mode.
                if hasattr(parent_agent, "_active_children"):
                    try:
                        parent_agent._active_children.remove(child)
                    except (ValueError, AttributeError):
                        pass  # Child wasn't tracked — fine
                return json.dumps(
                    {
                        "status": "dispatched",
                        "delegation_id": dispatch["delegation_id"],
                        "message": (
                            f"Background subagent dispatched. Goal: {_t['goal'][:100]}. "
                            f"It will run asynchronously and re-enter the conversation "
                            f"when complete. Use /agents to see active delegations."
                        ),
                    },
                    ensure_ascii=False,
                )
                ...
                return tool_error(
                    f"Async dispatch rejected: {dispatch.get('error', 'unknown')}"
                )
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'logos auth'."
        )
...
DELEGATE_TASK_SCHEMA = {
    ...
    "description": (
        ...
        "Only the final summary returns — intermediate tool results never enter your context.\n\n"
        "TWO MODES (one of 'goal' or 'tasks' is required): "
        "1. Single task: 'goal' (+ optional context, toolsets). "
        "2. Batch (parallel): 'tasks' array, up to delegation.max_concurrent_children items (default 3), "
        "all concurrent, results returned together. Nested delegation requires role='orchestrator' and max_spawn_depth >= 2.\n\n"
        "USE FOR: reasoning-heavy subtasks (debugging, code review, research synthesis); "
        "tasks that would flood your context with intermediate data; parallel independent workstreams. "
        "DON'T USE FOR: mechanical multi-step work (use execute_code); a single tool call "
        "(call it directly); tasks needing user interaction (subagents can't clarify); "
        "work that must outlive this turn (use cronjob or terminal background+notify — "
        "delegate_task runs SYNCHRONOUSLY and dies if the parent turn is interrupted).\n\n"
        "RULES:\n"
        "- Subagents have NO memory of this conversation — pass everything (paths, errors, "
        "constraints) via 'context'.\n"
        "- If the user wants a specific language/tone, say so in 'context' (subagents default to English).\n"
        "- Summaries are SELF-REPORTS, not verified facts. For operations with external "
        "side-effects, require a verifiable handle (URL, ID, path, HTTP status) and verify it "
        "yourself before reporting success.\n"
        "- role='leaf' (default) CANNOT call: delegate_task, clarify, memory, send_message, execute_code. "
        "role='orchestrator' retains delegate_task (bounded by delegation.max_spawn_depth, default 2; "
        "disable globally via delegation.orchestrator_enabled=false).\n"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "toolsets": {
                ...
                "items": {"type": "string"},
                "description": (
                    "Toolsets to enable for this subagent. Default: inherits yours. "
                    f"Available: {_TOOLSET_LIST_STR}. "
                    "Patterns: ['terminal','file'] code work; ['web'] research; "
                    "['browser'] web interaction; ['terminal','file','web'] full-stack."
                ),
            },
            "persona": {
                "type": "string",
                "description": (
                    "Optional persona name from ~/.hermes/personas/registry.yaml — "
                    "the child loads that persona's skill instructions and "
                    "toolset restrictions automatically."
                ),
            },
            "capability_mode": {
                "type": "string",
                "description": (
                    "Optional subagent toolset filter: 'readOnly' (web/search/file only), "
            ...
            "background": {
                "type": "boolean",
                "description": (
                    "Dispatch as a background subagent. When true (single-task only), "
                    "the parent returns immediately with a handle and the result "
                    "re-enters the conversation when complete. Default: false (synchronous). "
