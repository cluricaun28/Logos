#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Logos Skills Hub.
...
# ---------------------------------------------------------------------------
# Logos centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Logos index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Logos index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class LogosIndexSource(SkillSource):
    """Skill source backed by the centralized Logos Skills Index.
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        LogosIndexSource(auth=auth), # Centralized index (search + resolved install paths)
