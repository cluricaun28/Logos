...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from logos_cli.plugins import invoke_hook
...
# docker) already bypass the dangerous-command layer
...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(?:hermes|logos)\s+gateway\s+(stop|restart)\b', "stop/restart logos gateway (kills running agents)"),
    (r'\b(?:hermes|logos)\s+update\b', "logos update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart hermes-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart hermes-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hermes|logos|gateway|cli\.py)\b', "kill hermes/logos/gateway process (self-termination)"),
