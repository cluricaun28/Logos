...
`mcp_tool.py`, and `logos_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# LogosMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class LogosMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`LogosTokenStorage` persisting an absolute
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`LogosMCPOAuthProvider` directly using the helpers
        ...
        storage = LogosTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``logos mcp remove <name>`` and (indirectly) by
        ``logos mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
