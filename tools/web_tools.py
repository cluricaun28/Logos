...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "local":
        return _is_local_stack_available()
    if backend == "exa":
...
def _is_local_stack_available() -> bool:
    """Check if at least SearXNG and local Firecrawl are reachable."""
...
def _local_searxng_search(query: str, limit: int) -> dict[str, Any]:
    """Search via local SearXNG instance.

    Returns the standard web_search response format.
    """
    import httpx as _httpx  # noqa: F811

    resp = _httpx.post(
        f"{LOCAL_SEARXNG_URL}/search",
...
def _camofox_scrape(url: str) -> dict[str, Any]:
    """Scrape a URL via Camofox browser (Tier 3 fallback).

    Navigates the URL in a real browser and extracts page content.
    Returns a dict with 'markdown' key if successful, or raises.
    """
    import httpx as _httpx  # noqa: F811
    ...
    try:
        ...
        text = extract_resp.json().get("result", "")
...
def _local_extract_with_fallback(url: str) -> dict[str, Any]:
    """Extract content from a URL using local Firecrawl, falling back to Camofox.

    Firecrawl handles most pages. Camofox is the Tier 3 fallback for
    JS-rendered pages, login walls, or sites that block scraping.
    """
    try:
...
def _get_parallel_client():
    ...
    if _parallel_client is None:
        ...
        if not api_key:
            raise ValueError("PARALLEL_API_KEY environment variable not set. Get your API key at https://parallel.ai")
...
def _tavily_request(endpoint: str, payload: dict) -> dict:
    ...
    if not api_key:
        raise ValueError("TAVILY_API_KEY environment variable not set. Get your API key at https://app.tavily.com/home")
...
async def web_extract_tool(
    ...
    **kwargs,
    ...
    debug_call_data = {
        "parameters": {"urls": urls, "format": format, "use_llm_processing": use_llm_processing, "model": model, "min_length": min_length},
        "error": None,
        "pages_extracted": 0,
    ...
    try:
        ...
        if not safe_urls:
            ...
            if backend == "parallel":
                ...
                logger.info("Local extract (Firecrawl + Camofox): %d URL(s)", len(safe_urls))
                results = []
                        ...
                        results.append(
                            {
                                ...
                                "title": "",
                                "content": "",
                                "error": scraped.get("error", "Extraction failed"),
                                "raw_content": "",
    ...
    **kwargs,
    ...
    try:
        ...
        if backend == "tavily":
            ...
            if not is_safe_url(url):
                return json.dumps(
                    {"results": [{"url": url, "title": "", "content": "", "error": "Blocked: URL targets a private or internal network address"}]},
                    ensure_ascii=False,
                )
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        print(f"Set EXA_API_KEY, PARALLEL_API_KEY, TAVILY_API_KEY, FIRECRAWL_API_KEY, FIRECRAWL_API_URL{_firecrawl_backend_help_suffix()}")
...
registry.register(
    ...
    handler=lambda args, **kw: web_extract_tool(args.get("urls", [])[:5] if isinstance(args.get("urls"), list) else [], "markdown"),
    check_fn=check_web_api_key,
    requires_env=_web_requires_env(),
    is_async=True,
    emoji="📄",
