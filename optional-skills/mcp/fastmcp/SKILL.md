...
- smoke-test a server with the FastMCP CLI before wiring it into Logos or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Logos. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ${LOGOS_HOME:-${HERMES_HOME:-$HOME/.logos}}/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ${LOGOS_HOME:-${HERMES_HOME:-$HOME/.logos}}/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Logos integration, either:

- configure the server in `~/.logos/config.yaml` (legacy `~/.hermes/config.yaml`)` using the `native-mcp` skill, or
...
### Logos cannot see the deployed server

The server-building part may be correct while the Logos config is not. Load the `native-mcp` skill and configure the server in `~/.logos/config.yaml`, then restart Logos.
