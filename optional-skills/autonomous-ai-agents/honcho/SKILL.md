---
name: honcho
description: Configure and use Honcho memory with Logos -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Logos
license: MIT
metadata:
...
# Honcho Memory for Logos

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Logos profile its own peer identity while sharing a unified view of the user.
...
```bash
logos honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Logos profile's AI peer
...
Honcho models conversations as interactions between **peers**. Logos creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Logos instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Logos run |
| `global` | Single session across all directories |

Manual override: `logos honcho map my-project-name`
...
Each Logos profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
logos profile create coder --clone
...
```bash
logos honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Logos when Honcho memory is active.
...
Config file: `$LOGOS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `logos honcho setup`. Ensure `memory.provider: honcho` is in `~/.logos/config.yaml` (legacy `~/.hermes/config.yaml`)`.

### Memory not persisting across sessions
Check `logos honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `logos profile create <name> --clone`. For existing profiles: `logos honcho sync`.
...
| Command | Description |
|---------|-------------|
| `logos honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `logos honcho status` | Show resolved config, connection test, peer info for active profile |
| `logos honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `logos honcho disable` | Disable Honcho for the active profile |
| `logos honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `logos honcho peers` | Show peer identities across all profiles |
| `logos honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `logos honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `logos honcho sessions` | List known directory-to-session-name mappings |
| `logos honcho map <name>` | Map current working directory to a Honcho session name |
| `logos honcho identity` | Seed AI peer identity or show both peer representations |
| `logos honcho sync` | Create host blocks for all Logos profiles that don't have one yet |
| `logos honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Logos + Honcho |
| `logos memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `logos memory status` | Show active memory provider and config |
| `logos memory off` | Disable external memory provider |
