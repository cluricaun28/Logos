...
def remove_path_from_shell_configs():
    """Remove Logos PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Logos" / legacy "# Hermes" comment and following line
                if '# Logos' in line or '# Hermes' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``logos gateway run`` processes
    ...
    try:
        from logos_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from logos_cli.gateway import get_launchd_plist_path
...
def _discover_named_profiles():
    ...
    default root."""
    try:
        from logos_cli.profiles import list_profiles
    except (ImportError, ModuleNotFoundError):
        return []
    try:
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m logos_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "logos_cli.main", "--profile", name]
...
def run_uninstall(args):
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Logos data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Logos code but keep your configuration and data.")
