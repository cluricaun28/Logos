...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "logos_cli.main --profile",
        "logos_cli.main -p",
        "logos_cli/main.py gateway",
        "logos_cli/main.py --profile",
        "logos_cli/main.py -p",
        "logos gateway",
        "gateway/run.py",
    ]
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``logos update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Logos profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}logos gateway status\n"
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `logos gateway`")
...
from logos_constants import is_container, is_termux, is_wsl
...
SERVICE_DESCRIPTION = "Logos Gateway - Messaging Platform Integration"
...
# Legacy service names from older Logos installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Logos gateway units.

    Detects unit files installed by older Logos versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Logos gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Logos gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Logos gateway unit(s) detected from an older install:")
    ...
    print_info("    logos gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Logos gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Logos gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: logos gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo logos gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    logos gateway uninstall")
    print_info("    sudo logos gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Logos can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo logos gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo logos gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo logos gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Logos often sets ``HOME`` to a profile-scoped directory, but
...
def _logos_home_for_target_user(target_home_dir: str) -> str:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        hermes_home = _logos_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m logos_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Logos install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}logos gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}logos gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `logos gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}logos gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}logos gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}logos gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}logos gateway start{scope_flag}")
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Logos install")
    return True
...
def launchd_stop():
    ...
    # `logos gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Logos install")
    else:
        print("⚠ Service definition is stale relative to the current Logos install")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: logos gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                print_info("  For persistence:   tmux new -s hermes 'logos gateway run'")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: logos gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: logos gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s hermes 'logos gateway run'")
                print()
            ...
            print()
            print("  logos gateway run                              # direct foreground")
            print("  tmux new -s hermes 'logos gateway run'         # persistent via tmux")
            print("  nohup logos gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: logos gateway run")
            sys.exit(0)
        else:
        ...
        if is_termux():
            ...
            print("Stop manual runs with: logos gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: logos gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: logos gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  sudo logos gateway install --system")
            else:
                ...
                if is_termux():
                    print("  nohup logos gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'logos gateway run'         # persistent via tmux")
                    print("  nohup logos gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  logos gateway install  # Install as user service")
                    print("  sudo logos gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy Logos gateway unit files from
