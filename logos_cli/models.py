...
Add, remove, or reorder entries here — both `logos setup` and
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `logos model`
    flow without maintaining a separate static list.
    """
    from logos_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Logos refreshes the cache from
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from logos_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  logos model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `logos model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `logos model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``logos model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``logos auth add openai-codex`` without
    ``logos model``).
    """
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from logos_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from logos_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from logos_constants import AI_GATEWAY_BASE_URL
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``logos model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from logos_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from logos_cli.model_switch import MODEL_ALIASES
    except (ImportError, ModuleNotFoundError):
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Logos' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Logos should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``logos auth add copilot`` and by ``_seed_from_env``
    ...
    """
    try:
        from logos_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from logos_cli.auth import read_credential_pool
        from logos_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Logos release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Logos release). Falls back to the hardcoded catalog if no token
...
def probe_lmstudio_models(
    ...
    try:
        ...
        return None
    except (ImportError, json.JSONDecodeError, ModuleNotFoundError, OSError, PermissionError, ValueError) as exc:
        import logging
        logging.getLogger(__name__).debug(
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Logos' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Logos will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Logos cannot verify the model name."
