"""``logos logs`` — view and filter Logos log files.
...
Usage examples::
    ...
    logos logs                    # last 50 lines of agent.log
    logos logs -f                 # follow agent.log in real time
    logos logs errors             # last 50 lines of errors.log
    logos logs gateway -n 100    # last 100 lines of gateway.log
    logos logs --level WARNING    # only WARNING+ lines
    logos logs --session abc123   # filter by session ID substring
    logos logs --component tools  # only tool-related lines
    logos logs --since 1h         # lines from the last hour
    logos logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if component:
        from logos_logging import COMPONENT_PREFIXES
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'logos chat' to generate logs)")
