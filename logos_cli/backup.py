...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Logos backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a Logos backup "
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Logos configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # logos_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: logos profile list  (after installing Logos)")

        # Guidance
        print()
        if not (hermes_root / "hermes-agent").is_dir():
            print("Note: The hermes-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: logos update")
        ...
        print("Done. Your Logos configuration has been restored.")
...
# — `logos update` snapshots this set before pulling so approved-user lists
