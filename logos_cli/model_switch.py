...
- ``logos_cli.providers``        -- canonical provider identity + overlays
- ``logos_cli.model_normalize``  -- per-provider name formatting
...
from logos_cli.providers import (
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Logos. They lack the tool-calling capabilities "
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'logos model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'logos doctor' — config issues detected:"
    ...
    except (AttributeError, KeyError, OSError, RuntimeError, TypeError, Exception) as e:
        validation = {
            "accepted": False,
            "persist": False,
            "recognized": False,
    ...
    # Mirrors the same logic in logos_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from logos_cli/models.py (OPENROUTER_MODELS,
    ...
    # --- 1. Check Logos-mapped providers ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # show up in the picker without requiring a Logos release.
    ...
    # --- 2. Check Logos-only providers (nous, openai-codex, copilot, opencode-go) ---
    from logos_cli.providers import HERMES_OVERLAYS
    from logos_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Logos provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Logos IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve Logos slug — e.g. "github-copilot" → "copilot"
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    ...
                    has_creds = True
            except (AttributeError, ImportError, KeyError, ModuleNotFoundError, TypeError, Exception) as exc:
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # Use curated list — look up by Logos slug, fall back to overlay key
            model_ids = curated.get(hermes_slug, []) or curated.get(pid, [])
    ...
    # with `logos model`).
    try:
        from logos_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Logos's canonical write key (matches
            ...
            # Logos writes ``models:`` as a dict keyed by model id
            # (see logos_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Logos's own writer uses; a hyphen variant is accepted
            ...
            # active model. Logos's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, logos_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`logos_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from logos_cli.models import fetch_openrouter_models
