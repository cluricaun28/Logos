...
Toolsets = whatever the user has configured for "cli" in `logos tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `logos chat`:
...
def run_oneshot(
    ...
    env_model_early = logos_env("INFERENCE_MODEL", "").strip()
    ...
    os.environ["LOGOS_ACCEPT_HOOKS"] = "1"
...
def _run_agent(
    ...
    env_model = logos_env("INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            current_provider = (
                cfg_provider
                or logos_env("INFERENCE_PROVIDER", "").strip().lower()
                or "auto"
            )
