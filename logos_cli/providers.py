"""
Single source of truth for provider identity in Logos.
...
2. **Logos overlays** — transport type, auth patterns, aggregator flags,
...
# -- Logos overlay ----------------------------------------------------------
# Logos-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class LogosOverlay:
    """Logos-specific provider metadata layered on top of models.dev."""
...
HERMES_OVERLAYS: Dict[str, LogosOverlay] = {
    "openrouter": LogosOverlay(
    ...
    "openai-codex": LogosOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": LogosOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HERMES_QWEN_BASE_URL",
    ),
    "google-gemini-cli": LogosOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": LogosOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": LogosOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": LogosOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": LogosOverlay(
    ...
    "kimi-for-coding": LogosOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": LogosOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": LogosOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": LogosOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": LogosOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": LogosOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": LogosOverlay(
        transport="openai_chat",
    ...
    "opencode": LogosOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": LogosOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Logos overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Logos overlay
    ...
    if overlay is not None:
        # Logos-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
