...
TIPS = [
    ...
    "Ctrl+Z suspends Logos to the background — run fg in your shell to resume.",
    ...
    "logos chat -t web,terminal enables only specific toolsets for a focused session.",
    "logos chat -s github-pr-workflow preloads a skill at launch.",
    "logos chat -q \"query\" runs a single non-interactive query and exits.",
    "logos chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "logos chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "logos --yolo bypasses all dangerous command approval prompts for the entire session.",
    "logos chat --source telegram tags the session for filtering in hermes sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "logos doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hermes dump outputs a compact setup summary — great for bug reports.",
    "logos config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "logos config edit opens config.yaml in your default editor.",
    "logos config check scans for missing or stale configuration options.",
    ...
    "logos skills search react --source skills-sh searches the skills.sh public directory.",
    "logos skills check scans installed hub skills for upstream updates.",
    "logos skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "logos skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "logos mcp add github --command npx adds MCP servers from the command line.",
    "logos mcp serve runs Logos itself as an MCP server for other agents.",
    "logos auth add lets you add multiple API keys for credential pool rotation.",
    "hermes completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "logos logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hermes backup creates a zip backup of your entire Logos home directory.",
    "logos profile create coder creates an isolated profile that becomes its own command.",
    "logos profile create work --clone copies your current config and keys to a new profile.",
    "logos update syncs new bundled skills to ALL profiles automatically.",
    "logos gateway install sets up Logos as a system service (systemd/launchd).",
    "logos memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Logos tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'logos profile create coder' creates the 'coder' command.",
    "logos profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Logos prints a resume command with session ID and stats.",
    ...
    "logos skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Gateway & Messaging ---
    "Logos runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "logos gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Logos via a local macOS server.",
    ...
    "Context files (LOGOS.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.logos/SOUL.md (~/.hermes pre-rebrand) is the agent's primary identity — customize it to shape behavior.",
    "Logos loads project context from LOGOS.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — logos tools can toggle them per platform.",
    ...
    "logos chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "logos plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "logos acp runs Logos as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "logos login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Logos your own.",
    ...
    "Before a gateway session reset, Logos auto-flushes important facts to memory in the background.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'logos profile create ls' would be rejected.",
    "logos profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "logos claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via logos tools.",
