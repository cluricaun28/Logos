...
That was one user. Logos started with a single owner and has since **scaled to a dozen-plus users, each with their own private memory, reference library, and worldview profile**. The scaling is not a dozen-plus users crammed into one card: every model instance runs on its own GPU, and the deployment grows by adding instances. The dozen-plus-user deployment is a single 8-GPU server (96GB per GPU); the 32GB single-GPU box is the single-user reference configuration, not the multi-user one.

Dated snapshot, **as of 2026-09-16 — and growing as it is used**: 55,000+ conversation turns stored verbatim across 5,000+ sessions; a 40,000+ page searchable knowledge library (curated reference + public-domain archive + operational records). Built on my home PC starting April 2026 — I built the core of the system (Perpetual Memory, Reference Library, the rolling-window context engine, local vLLM inference) directly into the hermes-agent framework (Nous Research, since mid-2025), then forked it — and have been in continuous use since; since scaled from that home PC to a multi-GPU server.
...
| RL live viewer (rl_viewer.py) | 8090 | Reference Library browser (on-demand render, never stale) |
