...
from logos_cli.timeouts import (
...
_loaded_env_paths = load_logos_dotenv(logos_home=_logos_home, project_env=_project_env)
...
def _routermint_headers() -> dict:
    ...
    from logos_cli import __version__ as _HERMES_VERSION

    return {
        "User-Agent": f"LogosAgent/{_HERMES_VERSION}",
    }
...
class AIAgent:
    ...
    def __init__(
        ...
        self._cache_ttl = "5m"
        try:
            from logos_cli.config import load_config as _load_pc_cfg
        ...
        from logos_logging import setup_logging, setup_verbose_logging
        setup_logging(logos_home=_logos_home)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from logos_cli.config import load_config as _load_br_cfg
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `logos model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `logos model` to "
                        "select a provider, or run `logos setup` for first-time "
                        "configuration."
                    )
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from logos_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from logos_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Logos.  Choose a model with at least "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Logos' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from logos_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from logos_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_archiver") and self.context_archiver:
            ...
            try:
                from logos_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``logos chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `logos setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Logos "
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from logos_cli.models import _should_use_copilot_responses_api
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Pointer to the hermes-agent skill + docs for user questions about Logos itself.
        ...
        from logos_time import now as _logos_now
        now = _logos_now()
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from logos_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from logos_cli.copilot_auth import resolve_copilot_token
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            try:
                from logos_cli.model_normalize import normalize_model_for_provider
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from logos_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached:
            return cached
        try:
            from logos_cli.models import lmstudio_model_reasoning_options
    ...
    def _needs_thinking_reasoning_pad(self) -> bool:
        """Check if the current provider needs reasoning_content padding.
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        try:
            from logos_cli.plugins import get_pre_tool_call_block_message
    ...
    def run_conversation(
        ...
        # ``logos logs --session <id>`` can filter a single conversation.
        from logos_logging import set_session_context
        ...
        # are reused.  The system prompt is Logos's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Logos internals.
            if effective_system:
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: logos login --provider nous")
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Legacy cleanup: logos config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: logos config set ANTHROPIC_API_KEY \"\"")
                    ...
                    # providers (DeepSeek, Kimi, MiMo, Logos).  A 429 can
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `logos auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: logos setup", force=True)
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    # If a destructive tool terminates or restarts Logos
