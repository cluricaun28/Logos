"""
Logos MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Logos-specific extra)

Usage:
    logos mcp serve
    logos mcp serve --verbose
...
class EventBridge:
    ...
    This is the Logos equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Logos MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "Logos messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Logos MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
