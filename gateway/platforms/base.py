...
# Audio file extensions Logos recognizes for native audio delivery.
...
IMAGE_CACHE_DIR = get_logos_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; LogosAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_logos_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_logos_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_logos_dir("cache/documents", "document_cache")
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from logos_cli.commands import should_bypass_active_session
